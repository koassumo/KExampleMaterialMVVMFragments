...
import com.example.kexamplematerialmvvmfragments.ui.fragments.BottomNavFragment
...
class RvAdapterBottomNav(fragmentContext: Context) : RecyclerView.Adapter<RvAdapterBottomNav.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(adapterOnlyOneItemData: NoteRv) {
            ...
            itemView.setOnClickListener {
                ...
                when (ttt) {
                    "Bottom Nav 1" -> activity?.supportFragmentManager?.beginTransaction()?.replace(
                        R.id.container, BottomNavFragment()
                    )?.addToBackStack(null)?.commit()
                    "Bottom Nav 2" -> activity?.supportFragmentManager?.beginTransaction()?.replace(
                        R.id.container, BottomNavFragment()
                    )?.addToBackStack(null)?.commit()
                    "Bottom Nav 3" -> activity?.supportFragmentManager?.beginTransaction()?.replace(
                        R.id.container, BottomNavFragment()
                    )?.addToBackStack(null)?.commit()
                }
            }
    ...
    override fun onBindViewHolder(holder: RvAdapterBottomNav.ViewHolder, position: Int) {
